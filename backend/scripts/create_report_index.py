#!/usr/bin/env python
# backend/scripts/create_report_index.py
import logging
import argparse
import sys
import os
...
def create_student_reports_index():
    """Create the student reports index in Azure AI Search using REST API."""
    # Import settings
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Create student reports index in Azure AI Search")
    args = parser.parse_args()
    
    result = create_student_reports_index()
    
    if result:
        logger.info("Student reports index created successfully")
    else:
        logger.error("Failed to create student reports index")
        sys.exit(1)